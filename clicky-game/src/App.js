...
class App extends Component {
  ...
  handleSaveClick = id => {
    // Variable to hold the pokemon state.
    const tilez = this.state.pokemon;
    // Search through character pokemon to find the one that matches the clicked id.
  ...
  render() {
    const { message, score, pokemon, topScore } = this.state;
    return (
      <div className="fluid-container lodge h-100vh">
        ...
        <div className="d-flex justify-content-center main-content mx-auto padding-main flex-wrap row">
          {pokemon.map(({ id, pokename, image, clicked }) => (
            <Card
              key={id}
              id={id}
              name={pokename}
